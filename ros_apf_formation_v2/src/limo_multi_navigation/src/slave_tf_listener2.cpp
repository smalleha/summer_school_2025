...
double mul = 1; 
double min_distance = 1.0;  // 记录最近的障碍物距离
...
int main(int argc, char** argv){
  ...
  slave_vel =
    node.advertise<geometry_msgs::Twist>("/limo2/cmd_vel", 10);
  ros::Subscriber laser = node.subscribe("/limo2/scan", 1, laserCallBack);
  ...
  while (node.ok()){
    ...
    try{
      listener.lookupTransform("/limo2", "/limo1",
    ...
    try{
      listener.lookupTransform("/limo1", "/limo2",
...
void laserCallBack(const sensor_msgs::LaserScan::ConstPtr &laser_msg)
{
    min_distance = 1.0;  // 重置最小距离

    for (int i = 0; i < laser_msg.get()->intensities.size(); i++)
    {
        intensities[i] = laser_msg->ranges[i];
