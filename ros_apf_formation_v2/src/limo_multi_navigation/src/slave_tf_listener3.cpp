...
int main(int argc, char** argv){
  ...
  slave_vel =
    node.advertise<geometry_msgs::Twist>("/limo3/cmd_vel", 10);
  ros::Subscriber laser = node.subscribe("/limo3/scan", 1, laserCallBack);
  ...
  while (node.ok()){
    ...
    try{
      listener.lookupTransform("/limo3", "/limo1",
    ...
    try{
      listener.lookupTransform("/limo1", "/limo3",
